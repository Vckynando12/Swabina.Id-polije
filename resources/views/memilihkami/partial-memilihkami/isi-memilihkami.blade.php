...
<div class="container">
    ...
    <div class="why-choose-us-btn-container">
        @foreach($MK as $index => $item)
            <button class="btn why-choose-us-btn {{ $index == 0 ? 'active' : '' }}" data-target="#mk-{{ $item->id }}">
                {{ is_array($item->title) ? $item->title['id'] : $item->title }}
            </button>
        @endforeach
    </div>
    ...
    @foreach($MK as $index => $item)
        <div id="mk-{{ $item->id }}" class="why-choose-us-content-section {{ $index == 0 ? 'active' : '' }}">
            <img src="{{ asset('storage/' . $item->image) }}" class="img-fluid gambare-konten" 
                alt="{{ is_array($item->title) ? $item->title['id'] : $item->title }} Image">
            <h1 class="judule-konten">
                {{ is_array($item->title) ? $item->title['id'] : $item->title }}
            </h1>
            <p class="deskripsine-konten">
                {{ is_array($item->description) ? $item->description['id'] : $item->description }}
            </p>
        </div>
    @endforeach
</div>
