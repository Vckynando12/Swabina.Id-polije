...
@section('content')
<div class="container">
    <h1>Manage Digital Solution Carousel</h1>
...
<script>
    ...
    document.getElementById('carouselForm').addEventListener('submit', function(event) {
        ...
        var url = id ? `{{ route('admin.digitalsolution.carouselds.update', '') }}/${id}` : `{{ route('admin.digitalsolution.carouselds.store') }}`;
        ...
        .catch(error => {
            ...
            Swal.fire({
                title: 'Error!',
                text: error.message || 'An unexpected error occurred. Please try again later or contact support.',
                icon: 'error',
                confirmButtonText: 'OK'
            });
        });
        ...
        btn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`{{ route('admin.digitalsolution.carouselds.destroy', '') }}/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        }
