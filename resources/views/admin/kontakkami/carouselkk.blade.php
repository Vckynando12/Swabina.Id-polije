...
@section('content')
<div class="container">
    <h1>Manage Kontak Kami Carousel</h1>
...
<script>
    ...
    document.getElementById('carouselForm').addEventListener('submit', function(event) {
        ...
        var url = id ? `{{ route('admin.kontakkami.carouselkk.update', '') }}/${id}` : `{{ route('admin.kontakkami.carouselkk.store') }}`;
        ...
        btn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`{{ route('admin.kontakkami.carouselkk.destroy', '') }}/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        }
